...
def main():
    ...
    for i in range(1, 10):
        for no_branches_to_be_pruned in range(1, 3):  # prune one or two branches
            decision_tree_shannon_pruned = DecisionTree(train, objective=objective, gain_f="shannon").prune_tree(
                no_branches_to_be_pruned)
            decision_tree_gini_pruned = DecisionTree(train, objective=objective, gain_f="gini").prune_tree(
                no_branches_to_be_pruned)
            # TODO Random Forests
    ...
    plt.plot(no_nodes_dt_shannon_table, accuracy_dt_shannon_table, 'ro', label="Shannon - Test")
    plt.plot(no_nodes_dt_shannon_table, accuracy_dt_shannon_train, 'rx', label="Shannon - Train")
    plt.plot(no_nodes_dt_gini_table, accuracy_dt_gini_table, 'go', label="Gini - Test")
    plt.plot(no_nodes_dt_gini_table, accuracy_dt_gini_train, 'gx', label="Gini - Train")
    plt.gca().legend()
